...
/// Cache for rendered block heights (for viewport culling)
#[allow(dead_code)]
...
impl BlockHeightCache {
    ...
    #[allow(dead_code)]
    pub fn insert(&mut self, key: u64, height: f32) {
        if self.entries.len() >= self.max_entries {
            // Simple eviction
...
impl Default for BlockHeightCache {
    fn default() -> Self {
        Self::new(256)
    }
}
