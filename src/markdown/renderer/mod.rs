...
pub fn render_doc(
    ...
    scroll_to_top_pending: &mut bool,
) {
    ...
    let first_measurement_pass = viewport.is_first_measurement_pass();
    ...
    if *scroll_to_top_pending {
        scroll_area = scroll_area.vertical_scroll_offset(0.0);
    }

    scroll_area.show_viewport(ui, |ui, vis_rect| {
    ...
    *scroll_to_top_pending = false;
    ...
    if heights_changed {
        ...
        if !first_measurement_pass {
            ui.ctx().request_repaint();
        }
    }

    viewport.finish_measurement_pass();
}
